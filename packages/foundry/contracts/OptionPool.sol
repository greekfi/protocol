...
contract OptionPool {
    ...
    function deposit(bytes32 key, uint128 deltaL, uint64 newMask) external {
        ...
        if (p.liquidity > 0 && p.cellMask != newMask) {
            ...
            if (removed != 0) {
                for (uint8 i = 0; i < N; ++i) {
                    if ((removed & (uint64(1) << i)) != 0) cellL[i] -= p.liquidity;
                }
            }
            if (added != 0) {
                for (uint8 i = 0; i < N; ++i) {
                    if ((added & (uint64(1) << i)) != 0) cellL[i] += p.liquidity;
                }
            }
            p.cellMask = newMask;
        ...
        if (deltaL > 0) {
            ...
            for (uint8 i = 0; i < N; ++i) {
                if ((p.cellMask & (uint64(1) << i)) != 0) cellL[i] += deltaL;
            }
        }
    }
    ...
    function debitCellBank(uint8 cellId, uint256 loss0) external {
        ...
        uint256 L = cellL[cellId];
        if (L == 0) {
            // nobody opted in; route to protocol buffer if you want
    ...
    function pending(bytes32 key) external view returns (uint256) {
        ...
        uint256 dDebt = debtNow - p.debtSum0LastX128;
        uint256 netGrowth = dFee + dCredit;
        if (dDebt > netGrowth) netGrowth = 0;
        else netGrowth -= dDebt;
