...
export function OptionsTradePanel({ selectedOption, onClose }: OptionsTradePanelProps) {
  ...
  const considerationNeeded =
    action === "exercise" ? (amountBigInt * selectedOption.strike) / BigInt(10 ** 18) : 0n;
  ...
  const handleApproveCollateral = () => {
    ...
    approveCollateral({
      ...
      args: [factoryAddress as `0x${string}`, BigInt("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")],
    });
  };
    ...
    <div className="p-6 bg-black/80 border border-gray-800 rounded-lg shadow-lg">
      ...
      {(isExecuting || isActionPending || isActionSuccess) && (
        ...
        >
          ...
          {actionHash && (
            <div className="mt-2 text-xs text-gray-400 break-all">Tx: {actionHash}</div>
          )}
        </div>
      )}
