...
export function Redeem({ optionAddress }: RedeemActionProps) {
  ...
  const maxRedeemable = option.balances?.option && option.balances?.redemption
    ...
    <div className="p-6 bg-black/80 border border-gray-800 rounded-lg shadow-lg">
      ...
      <div className="space-y-2 mb-4">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="text-purple-300">{formatBalance(option.balances?.collateral, option.collateral.decimals)}</span>
        </div>
      </div>
      ...
      <button
        ...
        disabled={status === "working" || !amount || parseFloat(amount) <= 0 || option.isExpired || maxRedeemable === 0n}
