...
Coupling to the contracts is via `market-maker/factories.json` (per-chain factory address + deployment block), which mirrors the data generated into `core/abi/chains/*.ts`. Keep them in sync when contracts redeploy.
...
│   ├── scripts-js/               # JS helpers (deploy, keystore, generateTsAbis.js → writes ../core/abi/)
...
│   ├── factories.json            # Per-chain factory address; mirror of core/abi/chains/*.ts
...
│   └── yarn.lock                 # Per-subproject lockfile
│
│   (core/abi/ — generated by foundry/scripts-js/generateTsAbis.js, lives
│    inside core/ so the Vercel build doesn't need "outside-of-root" access.
│    deployedContracts.ts + chains/{mainnet,base,arbitrum,foundry,ink}.ts.)
│
...
# ABI generation — writes to ../core/abi/ (consumed directly by core, no copy needed)
