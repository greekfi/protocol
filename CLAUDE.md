...
Smart contracts, frontend, market maker, and docs all live in this repo as a Yarn workspace (managed by Corepack — run `corepack enable` once per machine).
...
├── market-maker/           # @greek/market-maker — Black-Scholes quote server (direct / bebop / relay modes)
...
├── package.json            # Workspace root — scripts delegate to core/market-maker
...
yarn install                # Installs foundry + core + market-maker workspaces
```
...
### Market maker (from repo root)
```bash
yarn direct                 # Standalone HTTP + WebSocket quote server
yarn bebop                  # Bebop RFQ client
yarn relay                  # Bebop price relay
yarn dev:direct / dev:bebop / dev:relay   # watch-mode variants
