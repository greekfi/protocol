...
Smart contracts, frontend, event-sync, and docs all live in this repo as a Yarn workspace (managed by Corepack — run `corepack enable` once per machine).

The market-maker (Black-Scholes quote server, RFQ maker) was extracted to its own private repo: [`greekfi/market-maker`](https://github.com/greekfi/market-maker). It deploys independently to Fly.io at `api.greek.finance`. The frontend talks to it over `https://`/`wss://` — no shared types, only the contract addresses are coupled.
...
├── package.json            # Workspace root — scripts delegate to core / event-sync
├── yarn.lock               # Single source of truth
└── .yarnrc.yml             # Corepack-managed, no vendored yarn binary
```

The market-maker lives in its own repo: [`greekfi/market-maker`](https://github.com/greekfi/market-maker).

## Commands

### One-time setup
```bash
corepack enable             # Yarn is managed by Corepack; version pinned in root package.json
yarn install                # Installs foundry + core + event-sync workspaces
```
...
yarn next:check-types       # tsc --noEmit against core
```
